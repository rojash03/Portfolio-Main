          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {skillCategories.map((category, index) => (
              <AnimatedSection key={index} animation="fade-up" delay={index * 120}>
                <div className="bg-textColor p-8 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 border border-gray-200">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {category.skills.map((skill, i) => (
                      <span 
                        key={i} 
                        className="skill-tag-shimmer px-4 py-2 text-sm bg-secondary bg-opacity-10 text-primary border border-secondary border-opacity-30 rounded-lg font-medium hover:bg-secondary hover:text-textColor transition-all duration-200"
                      >
                        {skill}
                      </span>
                    ))}
                ...
                <div className="bg-gray-50 p-8 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 text-center border border-gray-200">
                  <div className={`w-16 h-16 bg-${cert.color}-100 rounded-full flex items-center justify-center mx-auto mb-4`}>
                    <svg className={`w-8 h-8 text-${cert.color}-600`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={cert.color === "green" ? "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" : cert.color === "blue" ? "M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" : "M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zM21 5a2 2 0 00-2-2h-4a2 2 0 00-2 2v12a4 4 0 004 4h4a2 2 0 002-2V5z"} />
                    </svg>
                  </div>
                  <h3 className="text-lg font-bold text-primary mb-2">{cert.title}</h3>
                  <p className="text-gray-600 mb-4">{cert.desc}</p>
                  <span className={`bg-${cert.color}-100 text-${cert.color}-700 px-4 py-2 rounded-full text-sm font-medium`}>
                    {cert.status}
                  </span>
                </div>
              </AnimatedSection>
